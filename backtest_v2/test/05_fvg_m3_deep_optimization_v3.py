"""
05_fvg_m3_deep_optimization_v3.py

Sửa lỗi từ bản gốc:
  1. FIX logic wait_candles (chặn mọi cụm lệnh thay vì chỉ đảo chiều).
  2. Bổ sung cost_r = 0.15 trong compute_metrics để mô phỏng phí/spread.
...
def main():
    ...
    print("=" * 80)
    print("  05v3 - FVG M3 DEEP OPTIMIZATION (SPREAD/COST INCLUDED) — XAUUSD_M3  ")
    print("=" * 80)
    ...
    #  7. Export CSV
    # -----------------------------------------------------------------------
    RESULT_DIR = os.path.join(BASE_DIR, "..", "result", "05")
    os.makedirs(RESULT_DIR, exist_ok=True)
    out_path = os.path.join(RESULT_DIR, "05v3_fvg_m3_deep_strategies.csv")
...
def _export_equity_chart(df, atr, adx, ema_dict, registry, top_result, out_dir):
    ...
    fig.update_layout(
        title=(f"Equity Curve — 05v3 SPREAD FIXED Grid Search (XAUUSD_M3_v2_new)<br>"
    ...
    chart_path = os.path.join(out_dir, "05v3_fvg_m3_best_chart.html")
